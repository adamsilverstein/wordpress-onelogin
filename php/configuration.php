...
if ( !function_exists( 'add_action' ) ) {
	...
	function onelogin_saml_configuration_render() {
		$title = esc_html__("SSO/SAML Settings", 'onelogin-saml-sso');
		?>
			<div class="wrap">
				<?php screen_icon(); ?>
				<div class="alignleft">
				...
				<div class="alignright">
					<a href="<?php echo esc_url( get_site_url().'/wp-login.php?saml_metadata' ); ?>" target="blank"><?php echo esc_html__("Go to the metadata of this SP", 'onelogin-saml-sso');?></a><br>
					<a href="<?php echo get_site_url().'/wp-login.php?saml_validate_config'; ?>" target="blank"><?php echo esc_html__("Once configured, validate here your OneLogin SSO/SAML Settings", 'onelogin-saml-sso');?></a>
				</div>
	...
	function onelogin_saml_configuration() {
		...
		$helpText = '<p>' . esc_html__('This plugin provides single sign-on via SAML and gives users one-click access to their WordPress accounts from identity providers like OneLogin', 'onelogin-saml-sso') . '</p>' .
			'<p><strong>' . esc_html__('For more information', 'onelogin-saml-sso') . '</strong> '. esc_html__("access to the", 'onelogin-saml-sso').' <a href="https://onelogin.zendesk.com/hc/en-us/articles/201173454-Configuring-SAML-for-WordPress" target="_blank">'. esc_html__("Plugin Info", 'onelogin-saml-sso').'</a> ' .
		...
		add_settings_section('idp', esc_html__('IDENTITY PROVIDER SETTINGS', 'onelogin-saml-sso'), 'plugin_section_idp_text', $option_group);
		$idp_fields = array (
			'onelogin_saml_idp_entityid' => esc_html__('IdP Entity Id', 'onelogin-saml-sso') . ' *',
			'onelogin_saml_idp_sso' => esc_html__('Single Sign On Service Url', 'onelogin-saml-sso') . ' *',
			'onelogin_saml_idp_slo' => esc_html__('Single Log Out Service Url', 'onelogin-saml-sso'),
			'onelogin_saml_idp_x509cert' => esc_html__('X.509 Certificate', 'onelogin-saml-sso')
		);
		...
		add_settings_section('options', esc_html__('OPTIONS', 'onelogin-saml-sso'), 'plugin_section_options_text', $option_group);
		$options_fields = array (
			'onelogin_saml_autocreate' => esc_html__('Create user if not exists', 'onelogin-saml-sso'),
			'onelogin_saml_updateuser' => esc_html__('Update user data', 'onelogin-saml-sso'),
			'onelogin_saml_forcelogin' => esc_html__('Force SAML login', 'onelogin-saml-sso'),
			'onelogin_saml_slo' => esc_html__('Single Log Out', 'onelogin-saml-sso')
		);
		...
		add_settings_field('onelogin_saml_account_matcher', esc_html__('Match Wordpress account by', 'onelogin-saml-sso'), "plugin_setting_select_onelogin_saml_account_matcher", $option_group, 'options');

		add_settings_section('attr_mapping', esc_html__('ATTRIBUTE MAPPING', 'onelogin-saml-sso'), 'plugin_section_attr_mapping_text', $option_group);
		...
		add_settings_section('role_mapping', esc_html__('ROLE MAPPING', 'onelogin-saml-sso'), 'plugin_section_role_mapping_text', $option_group);
		$mapping_fields = array (
		...
		add_settings_section('customize_links', esc_html__('CUSTOMIZE ACTIONS AND LINKS', 'onelogin-saml-sso'), 'plugin_section_customize_links_text', $option_group);

		register_setting($option_group, 'onelogin_saml_customize_action_prevent_local_login');
		add_settings_field('onelogin_saml_customize_action_prevent_local_login', esc_html__('Prevent local login', 'onelogin-saml-sso'), "plugin_setting_boolean_onelogin_saml_customize_action_prevent_local_login", $option_group, 'customize_links');

		register_setting($option_group, 'onelogin_saml_customize_action_prevent_reset_password');
		add_settings_field('onelogin_saml_customize_action_prevent_reset_password', esc_html__('Prevent reset password', 'onelogin-saml-sso'), "plugin_setting_boolean_onelogin_saml_customize_action_prevent_reset_password", $option_group, 'customize_links');

		register_setting($option_group, 'onelogin_saml_customize_action_prevent_change_password');
		add_settings_field('onelogin_saml_customize_action_prevent_change_password', esc_html__('Prevent change password', 'onelogin-saml-sso'), "plugin_setting_boolean_onelogin_saml_customize_action_prevent_change_password", $option_group, 'customize_links');

		register_setting($option_group, 'onelogin_saml_customize_action_prevent_change_mail');
		add_settings_field('onelogin_saml_customize_action_prevent_change_mail', esc_html__('Prevent change mail', 'onelogin-saml-sso'), "plugin_setting_boolean_onelogin_saml_customize_action_prevent_change_mail", $option_group, 'customize_links');

		register_setting($option_group, 'onelogin_saml_customize_stay_in_wordpress_after_slo');
		add_settings_field('onelogin_saml_customize_stay_in_wordpress_after_slo', esc_html__('Stay in Wordpress after SLO', 'onelogin-saml-sso'), "plugin_setting_boolean_onelogin_saml_customize_stay_in_wordpress_after_slo", $option_group, 'customize_links');

		register_setting($option_group, 'onelogin_saml_customize_links_user_registration');
		add_settings_field('onelogin_saml_customize_links_user_registration', esc_html__('User Registration', 'onelogin-saml-sso'), "plugin_setting_string_onelogin_saml_customize_links_user_registration", $option_group, 'customize_links');

		register_setting($option_group, 'onelogin_saml_customize_links_lost_password');
		add_settings_field('onelogin_saml_customize_links_lost_password', esc_html__('Lost Password', 'onelogin-saml-sso'), "plugin_setting_string_onelogin_saml_customize_links_lost_password", $option_group, 'customize_links');


		add_settings_section('advanced_settings', esc_html__('ADVANCED SETTINGS', 'onelogin-saml-sso'), 'plugin_section_advanced_settings_text', $option_group);

		register_setting($option_group, 'onelogin_saml_advanced_settings_debug');
		add_settings_field('onelogin_saml_advanced_settings_debug', esc_html__('Debug Mode', 'onelogin-saml-sso'), "plugin_setting_boolean_onelogin_saml_advanced_settings_debug", $option_group, 'advanced_settings');

		register_setting($option_group, 'onelogin_saml_advanced_settings_strict_mode');
		add_settings_field('onelogin_saml_advanced_settings_strict_mode', esc_html__('Strict Mode', 'onelogin-saml-sso'), "plugin_setting_boolean_onelogin_saml_advanced_settings_strict_mode", $option_group, 'advanced_settings');

		register_setting($option_group, 'onelogin_saml_advanced_settings_sp_entity_id');
		add_settings_field('onelogin_saml_advanced_settings_sp_entity_id', esc_html__('Service Provider Entity Id', 'onelogin-saml-sso'), "plugin_setting_string_onelogin_saml_advanced_settings_sp_entity_id", $option_group, 'advanced_settings');

		$mapping_fields = array (
			'onelogin_saml_advanced_settings_nameid_encrypted' => esc_html__('Encrypt nameID', 'onelogin-saml-sso'),
			'onelogin_saml_advanced_settings_authn_request_signed' => esc_html__('Sign AuthnRequest', 'onelogin-saml-sso'),
			'onelogin_saml_advanced_settings_logout_request_signed' => esc_html__('Sign LogoutRequest', 'onelogin-saml-sso'),
			'onelogin_saml_advanced_settings_logout_response_signed' => esc_html__('Sign LogoutResponse', 'onelogin-saml-sso'),
			'onelogin_saml_advanced_settings_want_message_signed' => esc_html__('Reject Unsigned Messages', 'onelogin-saml-sso'),
			'onelogin_saml_advanced_settings_want_assertion_signed' => esc_html__('Reject Unsigned Assertions', 'onelogin-saml-sso'),
			'onelogin_saml_advanced_settings_want_assertion_encrypted' => esc_html__('Reject Unencrypted Assertions', 'onelogin-saml-sso')
		);
		foreach ($mapping_fields as $name => $description) {
			register_setting($option_group, $name);
		...
		add_settings_field('onelogin_saml_advanced_settings_sp_x509cert', esc_html__('Service Provider X.509 Certificate', 'onelogin-saml-sso'), "plugin_setting_string_onelogin_saml_advanced_settings_sp_x509cert", $option_group, 'advanced_settings');

		register_setting($option_group, 'onelogin_saml_advanced_settings_sp_privatekey');
		add_settings_field('onelogin_saml_advanced_settings_sp_privatekey', esc_html__('Service Provider Private Key', 'onelogin-saml-sso'), "plugin_setting_string_onelogin_saml_advanced_settings_sp_privatekey", $option_group, 'advanced_settings');
	}
	...
	function plugin_setting_string_onelogin_saml_idp_entityid() {
		echo '<input type="text" name="onelogin_saml_idp_entityid" id="onelogin_saml_idp_entityid"
			  ...
			  '<p class="description">'. esc_html__('Identifier of the IdP entity. ("Issuer URL")', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_string_onelogin_saml_idp_sso() {
		echo '<input type="text" name="onelogin_saml_idp_sso" id="onelogin_saml_idp_sso"
			  ...
			  '<p class="description">'. esc_html__('SSO endpoint info of the IdP. URL target of the IdP where the SP will send the Authentication Request. ("SAML 2.0 Endpoint (HTTP)")', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_string_onelogin_saml_idp_slo() {
		echo '<input type="text" name="onelogin_saml_idp_slo" id="onelogin_saml_idp_slo"
			  ...
			  '<p class="description">'. esc_html__('SLO endpoint info of the IdP. URL target of the IdP where the SP will send the SLO Request. ("SLO Endpoint (HTTP)")', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_string_onelogin_saml_idp_x509cert() {
		...
		echo esc_html( get_option('onelogin_saml_idp_x509cert') );
		echo '</textarea>';
		echo '<p class="description">'. esc_html__('Public x509 certificate of the IdP.  ("X.509 certificate")', 'onelogin-saml-sso');
	}
	...
	function plugin_setting_boolean_onelogin_saml_autocreate() {
		...
		echo '<input type="checkbox" name="onelogin_saml_autocreate" id="onelogin_saml_autocreate"
			  ...
			  '<p class="description">'. esc_html__('Auto-provisioning. If user not exists,  Wordpress will create a new user with the data provided by the IdP.<br>Review the Mapping section.', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_updateuser() {
		...
		echo '<input type="checkbox" name="onelogin_saml_updateuser" id="onelogin_saml_updateuser"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__('Auto-update. Wordpress will update the account of the user with the data provided by the IdP.<br>Review the Mapping section.', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_forcelogin() {
		...
		echo '<input type="checkbox" name="onelogin_saml_forcelogin" id="onelogin_saml_forcelogin"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__('Protect Wordpress and force the user to authenticate at the IdP in order to access', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_select_onelogin_saml_account_matcher() {
		...
		echo '<select name="onelogin_saml_account_matcher" id="onelogin_saml_account_matcher">
			  <option value="username" '.($value == 'username'?'selected="selected"':'').'>'. esc_html__("Username", 'onelogin-saml-sso').'</option>
			  <option value="email" '.($value == 'email'? 'selected="selected"':'').'>'. esc_html__("E-mail", 'onelogin-saml-sso').'</option>
			</select>'.
			'<p class="description">'. esc_html__("Select what field will be used in order to find the user account. If you select the 'email' fieldname the plugin will prevent that the user can change his mail in his profile.", 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_string_onelogin_saml_attr_mapping_role() {
		echo '<input type="text" name="onelogin_saml_attr_mapping_role" id="onelogin_saml_attr_mapping_role"
			  ...
			  '<p class="description">'. esc_html__("The attribute that contains the role of the user, For example 'memberof'. If Wordpress can't figure what role assign to the user, it will assign the default role defined at the general settings.", 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_customize_action_prevent_local_login() {
		...
		echo '<input type="checkbox" name="onelogin_saml_customize_action_prevent_local_login" id="onelogin_saml_customize_action_prevent_local_login"
			  '.($value ? 'checked="checked"': '').'>
			  <p class="description">'. esc_html__("Check it in order to disable the local login. After that only SAML logins will be allowed.", 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_customize_action_prevent_reset_password() {
		...
		echo '<input type="checkbox" name="onelogin_saml_customize_action_prevent_reset_password" id="onelogin_saml_customize_action_prevent_reset_password"
			  '.($value ? 'checked="checked"': '').'>
			  <p class="description">'. esc_html__("Check it in order to disable the ability of reset the password on Wordpress.", 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_customize_action_prevent_change_password() {
		...
		echo '<input type="checkbox" name="onelogin_saml_customize_action_prevent_change_password" id="onelogin_saml_customize_action_prevent_change_password"
			  '.($value ? 'checked="checked"': '').'>
			  <p class="description">'. esc_html__("Check it in order to disable the ability of change the password on Wordpress.", 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_customize_action_prevent_change_mail() {
		...
		echo '<input type="checkbox" name="onelogin_saml_customize_action_prevent_change_mail" id="onelogin_saml_customize_action_prevent_change_mail"
			  '.($value ? 'checked="checked"': '').'>
			  <p class="description">'. esc_html__("Check it in order to disable the ability of change the mail on Wordpress (we recommend that if you are using mail as the account matcher field.", 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_customize_stay_in_wordpress_after_slo() {
		...
		echo '<input type="checkbox" name="onelogin_saml_customize_stay_in_wordpress_after_slo" id="onelogin_saml_customize_stay_in_wordpress_after_slo"
			  '.($value ? 'checked="checked"': '').'>
			  <p class="description">'. esc_html__("If SLO and Force SAML login are enabled, after the SLO process you will be redirected to the WP main page and a SAML SSO process will start, to prevent that check this and you will stay at the Wordpress Login Form. ", 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_string_onelogin_saml_customize_links_user_registration() {
		echo '<input type="text" name="onelogin_saml_customize_links_user_registration" id="onelogin_saml_customize_links_user_registration"
			  ...
			  <p class="description">'. esc_html__("Override the user registration link. ", 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_string_onelogin_saml_customize_links_lost_password() {
		echo '<input type="text" name="onelogin_saml_customize_links_lost_password" id="onelogin_saml_customize_links_lost_password"
			  value= "'.get_option('onelogin_saml_customize_links_lost_password').'" size="80">
 			  <p class="description">'. esc_html__("Override the lost password link. (prevent reset password must be deactivated or always the SAMl SSO will be forced)", 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_advanced_settings_debug() {
		...
		echo '<input type="checkbox" name="onelogin_saml_advanced_settings_debug" id="onelogin_saml_advanced_settings_debug"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__('Enable it when your are debugging the SAML workflow. Errors and Warnigs will be showed.', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_advanced_settings_strict_mode() {
		...
		echo '<input type="checkbox" name="onelogin_saml_advanced_settings_strict_mode" id="onelogin_saml_advanced_settings_strict_mode"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__("If Strict mode is Enabled, then Wordpress will reject unsigned or unencrypted messages if it expects them signed or encrypted.
	...
	function plugin_setting_string_onelogin_saml_advanced_settings_sp_entity_id() {
		echo '<input type="text" name="onelogin_saml_advanced_settings_sp_entity_id" id="onelogin_saml_advanced_settings_sp_entity_id"
			  ...
			  '<p class="description">'. esc_html__("Set the Entity ID for the Service Provider. If not provided, 'php-saml' will be used.", 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_advanced_settings_nameid_encrypted() {
		...
		echo '<input type="checkbox" name="onelogin_saml_advanced_settings_nameid_encrypted" id="onelogin_saml_advanced_settings_nameid_encrypted"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__('The nameID sent by this SP will be encrypted.', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_advanced_settings_authn_request_signed() {
		...
		echo '<input type="checkbox" name="onelogin_saml_advanced_settings_authn_request_signed" id="onelogin_saml_advanced_settings_authn_request_signed"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__('The samlp:AuthnRequest messages sent by this SP will be signed.', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_advanced_settings_logout_request_signed() {
		...
		echo '<input type="checkbox" name="onelogin_saml_advanced_settings_logout_request_signed" id="onelogin_saml_advanced_settings_logout_request_signed"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__('The samlp:logoutRequest messages sent by this SP will be signed.', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_advanced_settings_logout_response_signed() {
		...
		echo '<input type="checkbox" name="onelogin_saml_advanced_settings_logout_response_signed" id="onelogin_saml_advanced_settings_logout_response_signed"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__('The samlp:logoutResponse messages sent by this SP will be signed.', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_advanced_settings_want_message_signed() {
		...
		echo '<input type="checkbox" name="onelogin_saml_advanced_settings_want_message_signed" id="onelogin_saml_advanced_settings_want_message_signed"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__('Reject unsigned samlp:Response, samlp:LogoutRequest and samlp:LogoutResponse received', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_advanced_settings_want_assertion_signed() {
		...
		echo '<input type="checkbox" name="onelogin_saml_advanced_settings_want_assertion_signed" id="onelogin_saml_advanced_settings_want_assertion_signed"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__('Reject unsigned saml:Assertion received', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_boolean_onelogin_saml_advanced_settings_want_assertion_encrypted() {
		...
		echo '<input type="checkbox" name="onelogin_saml_advanced_settings_want_assertion_encrypted" id="onelogin_saml_advanced_settings_want_assertion_encrypted"
			  '.($value ? 'checked="checked"': '').'>'.
			  '<p class="description">'. esc_html__('Reject unencrypted saml:Assertion received', 'onelogin-saml-sso').'</p>';
	}
	...
	function plugin_setting_string_onelogin_saml_advanced_settings_sp_x509cert() {
		...
		echo esc_html( get_option('onelogin_saml_advanced_settings_sp_x509cert') );
		echo '</textarea>';
		echo '<p class="description">'. esc_html__('Public x509 certificate of the SP. Leave this field empty if you gonna provide the cert by the sp.crt', 'onelogin-saml-sso');
	}
	...
	function plugin_setting_string_onelogin_saml_advanced_settings_sp_privatekey() {
		...
		echo esc_html( get_option('onelogin_saml_advanced_settings_sp_privatekey') );
		echo '</textarea>';
		echo '<p class="description">'. esc_html__('Private Key of the SP. Leave this field empty if you gonna provide the private key by the sp.key', 'onelogin-saml-sso');
	}

	function plugin_section_idp_text() {
		echo "<p>". esc_html__("Set here some info related to the IdP that will be connected with our Wordpress. You can find this values at the Onelogin's platform in the Wordpress App at the Single Sign-On tab", 'onelogin-saml-sso')."</p>";
	}

	function plugin_section_options_text() {
		echo "<p>". esc_html__("In this section the behavior of the plugin is set.", 'onelogin-saml-sso')."</p>";
	}

	function plugin_section_attr_mapping_text() {
		echo "<p>". esc_html__("Sometimes the names of the attributes sent by the IdP not match the names used by Wordpress for the user accounts. In this section we can set the mapping between IdP fields and Wordpress fields. Notice that this mapping could be also set at Onelogin's IdP", 'onelogin-saml-sso')."</p>";
	}

	function plugin_section_role_mapping_text() {
		echo "<p>". esc_html__("The IdP can use it's own roles. Set in this section the mapping between IdP and Wordpress roles. Accepts multiple valued comma separated. Example: admin,owner,superuser", 'onelogin-saml-sso')."</p>";
	}

	function plugin_section_customize_links_text() {
		echo "<p>". esc_html__("When we enable the SAML SSO to be integrated with a IdP some Wordpress actions and links could be changed. In this section you will be able to enable or disable the ability of change the mail, change the password and reset the password. Also can override the user registration and the lost password links", 'onelogin-saml-sso')."</p>";
	}

	function plugin_section_advanced_settings_text() {
		echo "<p>". esc_html__("Handle some other parameters related to customizations and security issues.<br>If sign/encryption is enabled, then x509 cert and private key for the SP must be provided. There are 2 ways:<br>
